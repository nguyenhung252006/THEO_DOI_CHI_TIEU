...
function LocChiTieuTheoNam({ loaiChiTieu, year, isCLose }) {
    ...
    const Data = async () => {
        try {
            ...
            const dataMucChiTieu = dataChiTieu.filter(item => item.loaiChiTieu === loaiChiTieu && chuyenNgay(item.ngayTao).substring(0, 4) === year)
...
export default LocChiTieuTheoNam;