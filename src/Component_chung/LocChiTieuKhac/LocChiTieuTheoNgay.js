...
function LocChiTieuTheoNgay({ loaiChiTieu, ngayTao, isCLose }) {
    ...
    const Data = async () => {
        try {
            ...
            const dataMucChiTieu = dataChiTieu.filter(item => item.loaiChiTieu === loaiChiTieu && chuyenNgay(item.ngayTao) === ngayTao)
            const listMucChiTieu = dataMucChiTieu.map(item => ({
                tien: item.soTien,
                ghiChu: item.tenKhoan,
                id: item.id,
...
export default LocChiTieuTheoNgay;